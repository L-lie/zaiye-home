-- One Prompt sent to one target consumes one share unit, regardless of group size.
-- Every work earns one permanent share credit on its first public publication only.
-- Paid group membership is dormant; only site admins have unlimited sharing.

alter table public.yucang_prompt_shares
  add column if not exists charge_units integer;

update public.yucang_prompt_shares
set charge_units = recipient_count
where charge_units is null;

alter table public.yucang_prompt_shares
  alter column charge_units set default 1,
  alter column charge_units set not null,
  drop constraint if exists yucang_prompt_shares_charge_units_check,
  add constraint yucang_prompt_shares_charge_units_check
    check (charge_units between 1 and 1000),
  drop constraint if exists yucang_prompt_shares_charge_total_check,
...
as $$
begin
  if new.status = 'approved' and new.was_public and new.published_at is not null
    and not exists (
      select 1
      from public.yucang_share_credit_ledger entry
      where entry.user_id = new.author_id
        and entry.work_id = new.work_id
        and entry.event_type = 'publication_reward'
    )
  then
    insert into public.yucang_share_credit_ledger(
      user_id, delta, event_type, event_key, work_id, version_id, actor_id, details
    ) values (
      new.author_id, 1, 'publication_reward', 'publication-work:' || new.work_id::text,
      new.work_id, new.id, new.author_id,
      jsonb_build_object('workId', new.work_id, 'versionId', new.id, 'firstPublication', true)
...
begin
  ...
  if existing.id is not null then
    ...
    from public.yucang_prompt_shares shared where shared.sender_id = caller
      and (shared.created_at at time zone 'Asia/Shanghai')::date =
          (now() at time zone 'Asia/Shanghai')::date;
    remaining_free_shares := case when privileged then null else greatest(0, 3 - used_today) end;
    return next;
    return;
  end if;
...
comment on column public.yucang_prompt_shares.charge_units is
  'Billable share units. New friend and group sends both cost one unit; legacy rows preserve their original charge.';
comment on function public.yucang_get_share_credit_entitlement() is
  'Returns three daily share units plus durable publication/founder credits; only admins are unlimited.';
