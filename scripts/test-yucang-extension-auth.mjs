...
assert.match(pageSource, /async function getVerifiedWebsiteSession\(\)/);
assert.match(pageSource, /client\.auth\.getUser\(candidate\.access_token\)/);
assert.match(pageSource, /client\.auth\.refreshSession\(\)/);
assert.match(pageSource, /response\.status === 401 \|\| result\.error === "invalid_session"/);
assert.match(pageSource, /官网登录已过期，请重新选择登录方式。/);
...
assert.match(pageSource, /if \(flow\.consent === "accepted"\)/);
assert.match(pageSource, /checkbox\.checked = true/);
assert.match(pageSource, /loginConsent\.refresh\(\)/);
...
assert.match(sharedLogin, /data-last-used-method="github"/);
assert.match(sharedLogin, /data-last-used-method="google"/);
assert.match(sharedLogin, /data-last-used-method="email"/);
assert.match(pageSource, /applyLastLoginHint\(extensionLoginRoot/);
assert.match(pageSource, /localStorage\.getItem\(LAST_AUTH_EMAIL_KEY\)/);
assert.match(pageSource, /localStorage\.setItem\(LAST_AUTH_EMAIL_KEY, normalizedEmail\)/);
assert.match(pageSource, /sessionStorage\.setItem\(PENDING_AUTH_METHOD_KEY, provider\)/);
assert.match(pageSource, /sessionStorage\.removeItem\(PENDING_AUTH_METHOD_KEY\)/);
