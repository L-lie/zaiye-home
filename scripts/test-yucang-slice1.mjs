...
assert.match(appCss, /\.resource-grid \{ display: grid; grid-template-columns: repeat\(5[^}]*grid-auto-rows: 4px[^}]*grid-auto-flow: row dense/);
assert.match(appCss, /\.resource-card-image img \{[^}]*width: 100%; height: auto/);
assert.doesNotMatch(appCss, /\.resource-card\.has-image \.resource-card-image \{[^}]*aspect-ratio/);
assert.match(app, /function layoutResourceMasonry\(grid\)/);
assert.match(app, /card\.style\.gridRowEnd = value/);
assert.match(app, /layoutResourceMasonry\(grid\)/);
...
for (const asset of [
  ...
  "yucang/assets/featured/dark-gothic.jpg",
  "yucang/assets/featured/particle-poster.jpg",
